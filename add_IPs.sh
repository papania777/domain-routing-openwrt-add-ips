...
load_list_fast() {
    list_name="$1"
    set_name="$2"
    url_base="${3:-https://antifilter.download}"
    url="${url_base}/list/${list_name}.lst"
    tmp="/tmp/lst/${list_name}.lst"
    valid="/tmp/lst/${list_name}.valid"
    batch_file="/tmp/lst/batch.nft"
    mkdir -p /tmp/lst
...
setup_cron() {
    cmd="0 */12 * * * /etc/init.d/add-ip-subnet-routing start"
...
main() {
    echo "============================================================"
    echo "  Add IP/Subnet/Community Routing v4.6 (Stream-Safe)"
    echo "============================================================"
...
elif [ "${1:-start}" = "reload" ] || [ "${1:-start}" = "restart" ]; then
    "$0" stop
    sleep 1
    "$0" start
else
